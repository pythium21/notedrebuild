...
- **Secrets**: `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` as env vars (Railway Variables in production, `.env.local` locally, gitignored). No `config.js`-style file — that was the old repo's no-build-step workaround and no longer applies. Use `||` not `??` for env fallbacks (a `??` silently accepts an empty string, which an `||` fallback catches — matters at the Edge Runtime boundary).
