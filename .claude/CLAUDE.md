...
- **Complete/log/toggle actions on a list row** (not a form submit) use the same re-entry-guard shape, keyed by item id instead of a single boolean: a `Set<string>` of in-flight ids, added before the mutating call and removed in a `finally`, with the triggering row/button `disabled` while its id is in the set. See `src/app/page.tsx`'s `completingIds` (DECISIONS.md D-024) and `RecurringDetailPanel.tsx`'s per-type `loggingLabelId`/`isLoggingNumeric`/`deletingId` (DECISIONS.md D-022) for the two existing instances.
