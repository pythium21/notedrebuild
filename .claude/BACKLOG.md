...
- Entry Tracking (DECISIONS.md D-022) follow-ups: editing an existing item's tracking config (type/target/labels) from the detail panel — lib functions (`updateEntryConfig`, label CRUD) exist but there's no UI entry point yet; gym exercise/set logging (beyond a plain counter tap); an entry history view beyond the current-period log list; weekly/monthly target reset timezone handling if the user travels across timezones (current reset is local-device-clock based, same as `checklist.ts`'s existing streak logic — untested across a DST/timezone change).
