...
16. Today's undated-tasks fix (DECISIONS.md D-028, tester feedback 2026-08-29): no schema change, `tsc`/`build` clean — `getUndatedTasks()` (`src/lib/tasks.ts`) added to the Today section's task union in `reloadToday()` (`src/app/page.tsx`). Verify on device: an undated, unflagged task now appears in Today's "Today" section (indistinguishable from a dated-today row), stays there until completed/archived/dated, and giving it a date or completing it removes it from Today as expected.
