...
12. Service worker offline-fallback fix (2026-08-25, AUDIT.md finding #2 / MISTAKES.md): verify on a real device with airplane mode — after visiting a non-root route (e.g. `/tasks`) once online, going offline and reloading that same route (or opening the installed PWA to it) should still repaint the cached shell instead of the browser's default offline error page; a hard-refresh while online should still pick up a fresh deploy on every route, not just `/`.
