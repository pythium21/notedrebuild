...
2. Verify on a real device: does removing `autoFocus` (item 1's fix) resolve a keyboard-triggered, temporary version of the missing-bottom-nav symptom? Lower priority than item 3 below — the one actual on-device report received so far described a persistent (not keyboard-tied) symptom now attributed to item 3 instead, so this original theory is unconfirmed either way and may turn out to be moot.
3. Reverted 2026-08-08 (see MISTAKES.md): `interactiveWidget: 'resizes-content'` (`src/app/layout.tsx`) was added to fix the Notes slash-command menu being hidden behind the on-screen keyboard, but very likely broke the fixed bottom nav app-wide in the installed PWA instead (reported same day: bottom nav effectively unreachable, requires scrolling, only 2 of 5 items visible) — documented interaction between `interactive-widget=resizes-content` and `position: fixed` elements in standalone/installed PWA mode on Android. Removed rather than patched further, since I have no way to test either the original keyboard issue or this regression on a real device myself. Two things still open: (a) confirm on a real device that removing it actually restores the bottom nav, and (b) the original slash-menu-behind-keyboard problem (#6 in the Aug 6 batch) is unfixed again and needs a different, more scoped approach next time — e.g. a JS `visualViewport` listener that repositions just the block-menu popup, rather than a global viewport-level setting that affects every fixed-position element in the app.
...
- [ ] Fix URL overflow in list view — long URLs (e.g. LinkedIn URNs) run off-screen instead of truncating. The `og:title`-at-save-time option was built 2026-08-08 (DECISIONS.md D-015) and covers the common case; a "Refresh N link titles showing as raw URLs" button (also D-015, same day) backfills pre-fix rows on demand. Still best-effort though: a title-less save whose page has no `og:title`/`<title>`, is unreachable, or times out still falls back to the raw URL either way. CSS ellipsis/truncate is still worth adding as a hard guarantee against overflow regardless of title-fetch success.
